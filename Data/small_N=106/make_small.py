...
users = set();
for line in open("users_N=106.txt", 'r'):
	tem = line.split();
	users.add(int(tem[0]));
...
for cur_s in range(len(split)-1):
	...
	f_out = open("D-small-N=106-" + split[cur_s][0] + "--" + split[cur_s][1] + ".txt", 'w');
...
for cur_s in range(len(split)-1):
	...
	f_out = open("W-small-N=106-" + split[cur_s][0] + "--" + split[cur_s][1] + ".txt", 'w');
