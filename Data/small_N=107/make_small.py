...
users = set();
for line in open("users_N=107.txt", 'r'):
	tem = line.split();
	users.add(int(tem[0]));
...
for cur_s in range(len(split)-1):
	...
	f_out = open("D-small-N=107-" + split[cur_s][0] + "--" + split[cur_s][1] + ".txt", 'w');
...
for cur_s in range(len(split)-1):
	...
	f_out = open("W-small-N=107-" + split[cur_s][0] + "--" + split[cur_s][1] + ".txt", 'w');
